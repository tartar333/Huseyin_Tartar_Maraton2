...
public class MusteriRepository {
	...
	public MusteriRepository(MusteriService musteriService, Scanner scanner) {
	...
	public void musteriEkle() {
		...
		MusteriModel musteriModel = new MusteriModel(musteriID, ad, soyad, telefonNumarasi, mail);
		musteriService.musteriEkle(musteriModel);
	...
	private void musteriSil() {
		...
		while (true) {
			...
			if (musteriService.musteriIDVarMi(musteriID)) {
				...
				System.out.println("Bu ID ile bir müşteri bulunamadı. Önceki Menüye Dönüyorsunuz.");
				break;
			}
		}
	}
	...
	private void musterileriListele() {
		List<MusteriModel> musteriler = musteriService.musterileriListele();
		if (musteriler.isEmpty()) {
			System.out.println("Listelenecek müşteri bulunamadı.");
		} else {
			for (MusteriModel musteriModel : musteriler) {
				// Müşteri bilgilerini ekrana yazdırır
				System.out.println("Müşteri ID: " + musteriModel.getId() + ", İsim: " + musteriModel.getAdi() + ", Soyisim: " + musteriModel.getSoyadi() + ", Telefon: " + musteriModel.getTelefon() + ", Mail: " + musteriModel.getMail());
			}
		}
	}
	...
	private void musteriIDileAra() {
		while (true) {
			...
			MusteriModel musteriModel = musteriService.musteriIDileAra(musteriID);
	...
	private void musteriAdiIleAra() {
		while (true) {
			...
			List<MusteriModel> musteriler = musteriService.musteriIsimIleAra(isim);
			if (musteriler.isEmpty()) {
				System.out.println("Bu isimle müşteri bulunamadı.");
			} else {
				for (MusteriModel musteriModel : musteriler) {
					System.out.println(musteriModel);
				}
				break;
			}
		}
